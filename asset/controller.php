...
if (isset($_POST['reg_asset']))
{
    ...
    $mbr_address = mysqli_real_escape_string($myConnection, $_POST['asset_']);
    ...
    $mbr_password = "wadah123";
    // $date = mysqli_real_escape_string($myConnection, date('Y-m-d'));
    $check_ic = mysqli_query($myConnection, "SELECT * FROM `member` WHERE `mbr_ic` = '$mbr_ic'");
    if(mysqli_num_rows($check_ic) > 0){
        ...
        (`mbr_name`,`mbr_ic`,`mbr_address`,`mbr_gender`,`mbr_phone`,`mbr_dob`,`mbr_email`,`mbr_branch`,`mbr_password`)
	       VALUES 
        ('$mbr_name','$mbr_ic','$mbr_address','$mbr_gender','$mbr_phone','$mbr_dob','$mbr_email','$mbr_branch','$mbr_password')";
